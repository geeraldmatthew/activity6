class ClientWorkoutsController < ApplicationController
  ...
  def find
    ...
    #ClientWorkout.where("LOWER(client_name) = ?", params[:search_string].downcase)
    #simple downcase search

    #ClientWorkout.where(client_name: params[:search_string])
    #exact equal to search term^
  ...
  private
    ...
    def client_workout_params
      params.require(:client_workout).permit(:client_id, :trainer_id, :duration_mins, :date_of_workout, :paid_amount)
    end
end
